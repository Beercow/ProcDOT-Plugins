...
def parse_flow(IP):
    p = sub.Popen(['.\\plugins\\tcpflow', '-T %T--%A-%B', '-cgB', '-r', (os.getenv('PROCDOTPLUGIN_WindumpFilePcap'))], stdout=sub.PIPE, stderr=sub.PIPE)
...
def check_tcpflow_ver():
    try:
        p = sub.Popen(['.\\plugins\\tcpflow', '-V'], stdout=sub.PIPE, stderr=sub.PIPE)
...
def main():
    ...
    if os.getenv('PROCDOTPLUGIN_VerificationRun') == None:
        ...
        if os.getenv('PROCDOTPLUGIN_Name') == 'Extract Files From PCAP':
            p = sub.Popen(['.\\plugins\\tcpflow', '-T %N_%A-%B', '-o', (temp), '-ar', (os.getenv('PROCDOTPLUGIN_WindumpFilePcap'))], stdout=sub.PIPE, stderr=sub.PIPE)
            stdout, stderr = p.communicate()
...
#            p = sub.Popen(['.\\plugins\\tcpflow', '-T %N_%A-%B', '-o', (temp), '-ar', (os.getenv('PROCDOTPLUGIN_WindumpFilePcap'))], stdout=sub.PIPE, stderr=sub.PIPE)
#            p.wait()
            IP = os.getenv('PROCDOTPLUGIN_CurrentNode_name')[7:]
            IP = parse_IP(IP)
#            chunk_check(temp,IP)
