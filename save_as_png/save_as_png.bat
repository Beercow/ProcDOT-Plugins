@setlocal enabledelayedexpansion && py -x "%~f0" %* & exit /b !ERRORLEVEL!
#!/usr/bin/env python

from Tkinter import *
import tkFileDialog
import os
import subprocess as sub
import base64

def icon(tempFile):
    temp = os.getenv('LOCALAPPDATA')
    icon = \
    """ 
    """
    icondata= base64.b64decode(icon)
    tempFile= temp+"icon.ico"
    iconfile= open(tempFile,"wb")
    iconfile.write(icondata)
    iconfile.close()
    return tempFile

def gui(tempFile):
    root = Tk()
    root.withdraw() # hide root
    root.wm_iconbitmap(tempFile)
    name = tkFileDialog.asksaveasfilename(filetypes=[('PNG','*.png')], title='Save Graph as PNG', defaultextension='.png')
    if name:
        with open(name, 'w') as f:
            p = sub.Popen(['dot', (os.getenv('PROCDOTPLUGIN_GraphFileDot')), '-Tpng', '-Gdpi=300', '-o', (name) ], stdout=sub.PIPE, stderr=sub.PIPE)
            p.wait()

def main():
    tempFile = None
    tempFile = icon(tempFile)
    gui(tempFile)
    os.remove(tempFile)

if __name__ == '__main__':
    main()